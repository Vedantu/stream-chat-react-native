...
import iconNewMessage from '../images/icons/icon_new_message_2.png';
import iconAddAttachment from '../images/icons/plus-outline-2.png';
...
const Container = styled(({ padding, ...rest }) => <View {...rest} />)`
  ...
  padding-top: ${({ theme, padding }) => (padding ? theme.messageInput.container.conditionalPadding : 0)}px;
  margin-left: 10px;
  margin-right: 10px;
            ...
            class MessageInput extends PureComponent {
                ...
                sendMessage = () => {
                    ...
                    if (this.props.editing) {
                        ...
                        updatedMessage.mentioned_users = this.state.mentioned_users.map((mu) => mu.id);
                        // TODO: Remove this line and show an error when submit fails
                        this.props.clearEditingState();

                        const updateMessagePromise = this.props.client.updateMessage(updatedMessage).then(() => {
                            this.props.clearEditingState();
                        });
                ...
                _pickFile = async () => {
                    if (this.props.maxNumberOfFiles && this.state.numberOfUploads >= this.props.maxNumberOfFiles)
                        return;
                ...
                _uploadFile = async (id) => {
                    ...
                    try {
                        if (this.props.doDocUploadRequest) {
                            response = await this.props.doDocUploadRequest(file, this.props.channel);
                        } else {
                ...
                _uploadImage = async (id) => {
                    ...
                    try {
                        if (this.props.doImageUploadRequest) {
                            ...
                            response = await this.props.channel.sendImage(file.uri, null, contentType);
                        }
                    } catch (e) {
                        console.warn(e);
                        await this.setState((prevState) => {
                ...
                render() {
                                ...
                                <Container padding={this.state.imageUploads.length > 0}>
                                    {this.state.fileUploads && (
                                        <FileUploadPreview
                                            ...
                                            fileUploads={this.state.fileOrder.map((id) => this.state.fileUploads[id])}
                                        />
                                    )}
                                    ...
                                    <InputBoxContainer ref={this.props.setInputBoxContainerRef}>
                                        <AttachButton
                                            onPress={() => {
                                                if (hasImagePicker && hasFilePicker) this.attachActionSheet.show();
                  ...
                  */}
                                        ...
                                        <ActionSheet
                                            ...
                                            options={[
                                                /* eslint-disable */
                                                <AttachmentActionSheetItem icon={iconGallery} text="Upload a photo" />,
                                                <AttachmentActionSheetItem icon={iconFolder} text="Upload a file" />,
                                                /* eslint-enable */
                                            ]}
                                            onPress={(index) => {
                                        ...
                                        />
                                        <SendButton title="Pick an image from camera roll" onPress={this.sendMessage}>
                                            {this.props.editing ? (
